...
from ..core.camera_calibration import build_camera_calibration_text
from ..core.interpretation import interpret_indices
from ..core.new_data_profile import build_validation_profile_text
from .localization import translate_report_text
...
def build_interpretation_text(report: Mapping[str, Any], *, language: str = "ro") -> str:
    """Build the standalone Interpretation tab text."""

    return translate_report_text(_join_lines(_format_interpretation_section(report)), language)


def build_summary_text(
    report: Mapping[str, Any],
    selected_spot: Mapping[str, Any] | None = None,
    *,
    language: str = "ro",
) -> str:
    ...
    validation = _as_mapping(report.get("validation_profile"))
    metrics = _as_mapping(validation.get("reference_metrics"))
    if metrics:
        lines.append(
            _line(
                "Profil validare",
                (
                    f"r spectru {_format_float(metrics.get('inside_outside_median_shape_similarity_r'))}, "
                    f"semnal afara/cutie {_format_float(metrics.get('outside_to_inside_signal_ratio'), 1)}x"
                ),
            )
        )
    camera_calibration = _as_mapping(report.get("camera_calibration"))
    if camera_calibration.get("calibration_applied"):
        corrected = _as_mapping(camera_calibration.get("corrected_bands"))
        lines.append(_line("Corectie camera", f"{len(corrected)} benzi corectate"))
    if indices:
        lines.append(
            _line(
    ...
    return translate_report_text(_join_lines(lines), language)
...
def _format_whole_leaf_section(report: Mapping[str, Any]) -> list[str]:
    ...
    if corrected:
        formatted_corrected = []
        formatted_source_compensated = []
        for band, values in sorted(corrected.items(), key=lambda item: str(item[0])):
            value_map = _as_mapping(values)
            formatted_corrected.append(
                f"{band} {_format_float(value_map.get('white_normalized_camera'))}"
            )
            if value_map.get("source_curve_compensated_camera") is not None:
                formatted_source_compensated.append(
                    (
                        f"{band} {_format_float(value_map.get('source_curve_compensated_camera'))}"
                        f" (w={_format_float(value_map.get('source_curve_reliability_weight'))})"
                    )
                )
        lines.append(_line("Benzi corectate cu alb", ", ".join(formatted_corrected)))
        if formatted_source_compensated:
            lines.append(_line("Benzi compensate cu sursa", ", ".join(formatted_source_compensated)))
    if indices:
    ...
    *,
    ...
    lines = [
        ...
        *_format_whole_leaf_section(report),
        "",
        build_camera_calibration_text(report),
        "",
        *_format_interpretation_section(report),
        "",
        build_validation_profile_text(report),
        "",
