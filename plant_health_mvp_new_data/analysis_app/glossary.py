...
def _terms() -> list[TermDefinition]:
    ...
    terms = [
        ...
        TermDefinition(
            ...
            "Filtered camera bundles first isolate the foreground leaf/object; hyperspectral images prefer <code>NDVI &gt; threshold</code>. Fallbacks use NDVI-like, RGB excess green, or percentile threshold.",
        ...
        TermDefinition(
            "Vegetation and Masks",
            "Filtered camera foreground mask",
            "A camera-specific mask that keeps the best connected leaf/object component before calculating spectral averages.",
            "Avoiding lamp spill, box background, or filter glare when the filtered photos are used for calibration.",
            "Build normalized intensity scores from filter bands, threshold them with Otsu/percentiles, clean the mask, reject implausible components, and keep the best component.",
            "The selected pixels are the area used for camera band averages.",
            "It is a segmentation mask, not a health score.",
        ),
        TermDefinition(
            "Vegetation and Masks",
            "NDVI mask threshold",
    ...
    index_terms = [
        ...
        ("NDWI_850_940", "Water-sensitive normalized difference proxy.", "(NIR - WATER_BAND) / (NIR + WATER_BAND)", "NIR 850 and WATER_BAND 940.", "Higher/lower depends on water-band response and calibration.", "940 is marked low-confidence for spectrometer-calibrated conclusions in the current dataset; treat this as exploratory unless remeasured."),
        ("WATER_RATIO", "Ratio of water band to NIR.", "WATER_BAND / NIR", "940 and 850.", "Higher means 940 is larger relative to NIR.", "Sensitive to calibration, lighting, and the low-confidence 940 nm reference."),
