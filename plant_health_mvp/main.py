...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("plant_health_mvp") / "runs" / "latest",
        help="Directory where band images, mask, plot, and JSON report are written.",
    )
    parser.add_argument(
        "--save-mock-dataset",
        action="store_true",
        help="Also write a reusable mock_sample.npz and wavelengths.csv into plant_health_mvp/data/mock.",
    )
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.save_mock_dataset:
        mock_dataset_paths = save_mock_dataset(Path("plant_health_mvp") / "data" / "mock")
